 ...
 * * Problem Description:
 * We need to configure machines to reach specific target integer values (joltages).
 * Each button press adds +1 to specific counters.
 * We must find the non-negative integer number of presses for each button such that
 * all counters match their targets exactly, minimizing the total number of presses.
 * * Mathematical Model:
 ...
 * * Algorithm:
 * 1. Gaussian Elimination: Converts the matrix A to Reduced Row Echelon Form (RREF).
 * 2. Variable Classification: Identifies 'Pivot' variables (dependent) and 'Free' variables (independent).
 * 3. Recursive Search: Since the system is underdetermined, we iterate through possible 
 * values for the 'Free' variables. For each guess, we calculate the required values 
 ...
 */
 ...
 typedef struct {
     ...
     int pivot_col[MAX_VARS];      // Index of the pivot column for each row (-1 if none)
     int free_vars[MAX_VARS];      // List of column indices corresponding to free variables
     int num_free;                 // Count of free variables
 } System;
 
 // Tracks the global minimum sum of presses found across all valid solutions for a machine
 ...
 void search(System *sys, int free_idx, double *solution) {
     ...
     // Now we compute the dependent (Pivot) variables to see if this configuration works.
     if (free_idx == sys->num_free) {
         double current_sol[MAX_VARS];
         // Copy current guesses for free variables
         for(int i = 0; i < sys->cols; i++) current_sol[i] = solution[i];
         ...
         for (int r = 0; r < sys->rows; r++) {
             ...
             if (p == -1) {
                 if (fabs(sys->b[r]) > EPSILON) { valid = false; break; }
                 continue;
             }
         ...
         if (valid) {
             for (int i = 0; i < sys->cols; i++) {
                 if (current_sol[i] < 0) { valid = false; break; } // Safety check
     ...
     // In these puzzles, the required adjustment via free variables is typically small.
     for (int val = 0; val <= 200; val++) { 
         solution[f_var] = (double)val;
         
         // Optimization: We could add pruning here if the partial sum already exceeds global_min_presses,
         // but given the small depth (num_free is usually low), raw recursion is fast enough.
 ...
 void solve_system(System *sys) {
     ...
     for (int col = 0; col < sys->cols && pivot_row < sys->rows; col++) {
         ...
         if (sel == -1) continue; // Column is independent (Free Variable)
         ...
         for (int j = col; j < sys->cols; j++) sys->A[pivot_row][j] /= div;
         sys->b[pivot_row] /= div;
         ...
         for (int i = 0; i < sys->rows; i++) {
             if (i != pivot_row && fabs(sys->A[i][col]) > EPSILON) {
                 ...
                 for (int j = col; j < sys->cols; j++) sys->A[i][j] -= factor * sys->A[pivot_row][j];
     ...
     for(int i=0; i<sys->rows; i++) {
         if(fabs(sys->b[i]) < EPSILON) sys->b[i] = 0.0; // Clean small errors
 ...
 void parse_line(char *line, System *sys) {
     ...
     while ((p = strchr(p, '(')) != NULL) {
         if (brace && p > brace) break; // Don't parse inside {}
     ...
     if (brace) {
         ...
         if(end) {
             ...
             while(tok) {
                 sys->b[idx] = atof(tok);
                 if (idx >= sys->rows) sys->rows = idx + 1;
                 idx++;
                 tok = strtok(NULL, ",");
             }
         }
