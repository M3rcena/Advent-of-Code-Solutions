 ...
 * * Problem: Minimize the number of button presses to achieve a specific light configuration.
 * * Mathematical Model:
 * This problem describes a system of linear equations over the Finite Field GF(2).
 * - Each light represents an equation.
 * - Each button represents a variable (x_i) which can be 0 (unpressed) or 1 (pressed).
 ...
 */
 ...
 void read_line(char *line, System *sys) {
     ...
     while (*p && *p != ']') {
         if (*p == '#') sys->b[idx] = 1;
         else if (*p == '.') sys->b[idx] = 0;
         else { p++; continue; } // Skip extraneous chars
         idx++;
         p++;
     }
     sys->n = idx;
  ...
  * * Since the system might be underdetermined (multiple solutions), this function
  * employs a brute-force search over the "free variables" to find the solution
  * with the minimum Hamming weight (fewest button presses).
  * * @param sys The linear system.
  * @param sol Output array to store the best solution found (0s and 1s).
  * @return The rank of the matrix, or -1 if the system is inconsistent (no solution).
  */
 int gauss(System *sys, int *sol) {
     ...
     for (int col = 0; col < m && row < n; col++) {
         ...
         if (sel == -1) continue; // No pivot in this column, it's a free variable
         ...
         // Eliminate this column from all OTHER rows (making it a unit vector column)
         // This effectively performs Gauss-Jordan to simplify back-substitution later.
         for (int i = 0; i < n; i++) {
             if (i != row && sys->A[i][col]) {
                 for (int j = col; j < m; j++) {
                     sys->A[i][j] ^= sys->A[row][j]; // XOR is subtraction/addition in GF(2)
                 }
                 sys->b[i] ^= sys->b[row];
             }
         }
     ...
     // Changing a free variable requires adjusting dependent variables (pivots) to keep equations valid.
     int free_vars = 0;
     ...
     int min_weight = 0;
     for (int i = 0; i < m; i++) min_weight += sol[i];
     ...
     // If there are k free variables, there are 2^k valid solutions (the Null Space).
     // We expect 'free_vars' to be small (<= 15) for this optimization to run quickly.
     if (free_vars <= 15) { 
         for (int mask = 0; mask < (1 << free_vars); mask++) {
             ...
             // 2. Re-calculate dependent variables (pivots) based on free variable choices
             for (int i = 0; i < m; i++) {
                 if (where[i] != -1) { // If 'i' is a pivot variable
                     int val = sys->b[where[i]];
                     // The value of pivot i depends on the target b and the values of other cols in this row
                     for (int j = 0; j < m; j++) {
             ...
             int weight = 0;
             for (int i = 0; i < m; i++) weight += temp_sol[i];
 ...
 int solve_system(System *sys) {
     ...
     if (rank == -1) return -1; // No solution found
